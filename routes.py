...
#Home page, displays random mindful post from a user in addition to loading the home page.
@app.route('/')
def home():
...
#My post page, if user is logged in, it will display all posts by the current user, if not it will redirect them to the login page.
...
#Public post page, will display posts made by every user. (Add "sort by" functionality) 
...
#Admin page, locked by specific admin/password combination, will display all posts and all usernames.
...
#Allows the admin to ban users and remove their username and posts from the database.
@app.post('/ban_user')
def ban_user():
    conn = sqlite3.connect("mindful.db")
    cur = conn.cursor()
...
#Allows users to "like" other posts made by other users in the public posts page. (Implement fix for infinite "likes")
@app.post('/like') 
def like():
    conn = sqlite3.connect("mindful.db")
    cur = conn.cursor()
...
#Allows the user to create or login into thier account using thier username and password.
...
def create_or_login():
    ...
    if create_or_login == "create_account":
        ...
        #If it doesnt exist then it will insert it into the database and the username session is filled.
        if username == None: 
    ...
    if create_or_login == "login":
        ...
        #If it does exist the user can successfully login and redirected to their posts.
        else:
            session["username"] = request.form['username']
            return redirect('/my_posts')
